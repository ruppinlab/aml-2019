...
# conditional quantile normalization. Biostatistics 13, 204–216 (2012).
...
# select samples 
# After consulting with Ani & Karina, we decided to only include samples at initial diagnosis
rna.beat.clinical.sel2 <- rna.beat.clinical.sel1[rna.beat.clinical.sel1$specimenGroups 
                                                 %in% "Initial Acute Leukemia Diagnosis" & 
                                                   rna.beat.clinical.sel1$dxAtInclusion %in%
...
Merge datasets and plot PCA.
```{r}
# select LAML only
ix <- which(prob0$types=="LAML")
aml_tcga <- subset.prob.tcga(prob0, ix)

# free up some memory
rm(prob0)

# Convert FPKM to TPM
mrna.tpm <- apply(aml_tcga$mRNA, 2, function(x) fpkmToTpm(x))
aml_tcga$mrna.tpm <- mrna.tpm
...
# Merge TCGA and TARGET
ix <- intersect(row.names(aml_tcga$mrna.tpm), 
                row.names(aml.fpkms.prob.hg38$mrna.tpm))
# Intersect list with BEAT
iy <- intersect(ix, row.names(aml_beat$mrna.tpm))

# sort 
tcga <- aml_tcga$mrna.tpm[iy, ]
target <- aml.fpkms.prob.hg38$mrna.tpm[iy, ]
beat <- aml_beat$mrna.tpm[iy, ]

aml_cohort <- cbind(tcga, target, beat)

# Load function from below
pcas <- mat2pca(aml_cohort)
pcas1_4 <- as.data.frame(pcas[, 1:4])
pcas1_4$cohort <- c(rep("TCGA", dim(tcga)[2]), 
                    rep("TARGET", dim(target)[2]),
                    rep("BEAT", dim(beat)[2]))

pcas1_4$LabId_match_with_rpkm_matrix <- row.names(pcas1_4)

m.pcas <- merge(pcas1_4, 
                rna.beat.clinical, 
                by="LabId_match_with_rpkm_matrix",
                all.x = TRUE)
m.pcas$X..Blasts.in.BM <- as.numeric(m.pcas$X..Blasts.in.BM)

g <- ggplot(data=m.pcas, 
            aes(x=PC1, y=PC2, pch=cohort, colour=X..Blasts.in.BM))
...
# only select genes with an FPKM greater than 0 fpkm=log2(1=0)
...
p <- p + geom_point(fill="blue", colour="white", pch=21, alpha=0.5, size=2.5)
p <- p + geom_point(data=cox_mrna_m[cox_mrna_m$cntrl_padjust < 0.1, ], 
                    aes(x=log2(cntrl_HR), y=-log10(cntrl_P)),
                    fill="red", colour="white", pch=21, alpha=0.5, size=2.5)
p <- p + geom_text_repel(data=cox_mrna_m[cox_mrna_m$cntrl_padjust < 0.1 & log2(cox_mrna_m$cntrl_HR) > 0 , ], 
...
# save results
# write table
write.csv(cox_mrna, file=paste(dir.save, "cox_beat_ixdiag_tpm_results.csv", sep="/"), row.names=TRUE)
write.csv(cox_mrna, file=paste(dir.save, "cox_beat_ixdiag_rpkm_results.csv", sep="/"), row.names=TRUE)
