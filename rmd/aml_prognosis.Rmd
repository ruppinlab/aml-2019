...
rna.beat <- exp(as.matrix(rna.beat.fpkm[, 3:dim(rna.beat.rpkm)[2]]))
...
# Tyner et al., "Normalization was performed using the conditional quantile normalization procedure (64), which produced GC-content-corrected log2 reads per kilobase per million mapped reads (RPKM) values. This procedure produces both offsets to be used in conjunction with edgeR as well as a matrix of log2-normalized RPKM values for clustering"
...
rna.beat.clinical.sel2 <- rna.beat.clinical.sel1[rna.beat.clinical.sel1$specimenGroups %in%
                                                   sel.specimenGroups, ]
...
g <- ggplot(data=m.pcas, 
            aes(x=PC1, y=PC2, colour=X..Blasts.in.BM))
