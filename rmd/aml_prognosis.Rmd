...
rna.beat.fpkm <- read.csv(paste(dir.beat, "Gene_counts_RPKM.csv", sep=""), header=TRUE, sep=",", stringsAsFactors = FALSE)
...
rna.beat <- exp(as.matrix(rna.beat.fpkm[, 3:dim(rna.beat.fpkm)[2]]))
...
# Tyner et al., "Normalization was performed using the conditional quantile normalization procedure64, which produced GC-content-corrected log2 reads per kilobase per million mapped reads (RPKM) values. This procedure produces both offsets to be used in conjunction with edgeR as well as a matrix of log2-normalized RPKM values for clustering"
